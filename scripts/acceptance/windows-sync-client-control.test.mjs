...
it('parses only fixed Windows sync client actions', () => {
  ...
  expect(() => parseWindowsSyncClientArgs(['align', '--revision', 'a'.repeat(40)]))
...
it('pins the Windows dev checkout and exact isolated client revision', () => {
  const script = buildWindowsSyncClientPowerShell({
    action: 'start', instance: 'b', port: 9222, revision: 'b'.repeat(40)
  });
  expect(script).toContain("$branch -ne 'dev'");
  expect(script).toContain("$root -ne 'D:/C/foliole'");
