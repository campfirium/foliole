...
async function stopExistingClient({ endpoint = CDP, repoRoot, fetchApi = globalThis.fetch }) {
  let browser;
  try {
    browser = await chromium.connectOverCDP(endpoint, { timeout: 2_000 });
  } catch {
    return;
  }
  const expected = pathToFileURL(path.join(repoRoot, 'dist', 'desktop', 'index.html')).href;
  const page = browser.contexts()[0]?.pages().find((candidate) => candidate.url().startsWith(expected));
  if (!page) {
    await browser.close();
    throw new Error('Mac CDP port is not owned by this Foliole workspace');
  }
  ...
  while (Date.now() < deadline) {
    try {
      await fetchApi(`${endpoint}/json/version`, {
...
export async function openMacSyncClient({
  ...
  stop = stopExistingClient, fetchApi = globalThis.fetch
} = {}) {
  ...
  await stop({ endpoint: CDP, fetchApi, repoRoot });
