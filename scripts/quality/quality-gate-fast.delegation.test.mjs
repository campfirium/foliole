...
describe('quality-gate-fast.sh delegation', () => {
  ...
  it('delegates to the full gate when forced', async () => {
    ...
    try {
      await writePackageJson(tempRoot, {
        ...
        'test:windows:native-preview': 'node -e "console.log(\'full preview recovery ok\')"',
  ...
  it('delegates to the release gate when forced', async () => {
    ...
    try {
      await writePackageJson(tempRoot, {
        ...
        'test:windows:native-preview': 'node -e "console.log(\'release preview recovery ok\')"',
  ...
  it('caps Android path changes locally and defers hosted quality to scheduled T5', async () => {
    const tempRoot = await createQualityGateTempRoot();
    try {
      await writePackageJson(tempRoot, {
      ...
      expect(result.stdout).toContain('hosted quality deferred to scheduled T5');
