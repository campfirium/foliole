...
describe('quality-gate-fast.sh light related tests', () => {
  ...
  it('uses the light level for local component changes and runs adjacent tests', async () => {
    const tempRoot = await createQualityGateTempRoot();
    const typecheckMarker = toFixtureShellPath(path.join(tempRoot, 'typecheck.marker'));
    const lintMarker = toFixtureShellPath(path.join(tempRoot, 'lint.marker'));
    try {
      ...
      await writeFixtureFile(
        tempRoot,
        'src/features/image-cloze/components/ImageClozeCardView.test.tsx',
        'export {};\n'
      );

      const testFile = 'src/features/image-cloze/components/ImageClozeCardView.test.tsx';
      const result = await runQualityGate(tempRoot, {
      ...
      expect(await readFile(lintMarker, 'utf8')).toContain('src/features/image-cloze/components/ImageClozeCardView.tsx');
      expect(await readFile(typecheckMarker, 'utf8')).toBe('ok');
      expect(result.stdout).toContain(expectedVitestArgs('related test:', testFile));
      expect(result.stdout).not.toContain('repo lint should stay unused');
      expect(result.stdout).not.toContain('repo test should stay unused');
      expect(result.stdout).not.toContain('repo build should stay unused');
    } finally {
  ...
  it('prints a light route plan without running checks', async () => {
    const tempRoot = await createQualityGateTempRoot();
    try {
      await writePackageJson(tempRoot, {
        lint: 'node -e "console.log(\'repo lint should stay unused\')"',
      ...
      expect(result.stdout).toContain('[quality-gate-route] target: scoped lint + typecheck + related tests when present');
  ...
  it('prints related tests for light-level route json', async () => {
    ...
    try {
      ...
      expect(JSON.parse(result.stdout)).toMatchObject({
        level: 'light',
        target: 'scoped lint + typecheck + related tests when present',
        relatedTests: ['src/features/image-cloze/components/ImageClozeCardView.test.tsx']
      });
    } finally {
      await rm(tempRoot, { recursive: true, force: true });
    }
