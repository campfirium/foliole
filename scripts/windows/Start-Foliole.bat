...
wsl -d %DISTRO% --cd %PROJECT_DIR% bash -lc "pkill -f 'target/debug/foliole-tauri-core' >/dev/null 2>&1 || true; pkill -f 'tauri dev' >/dev/null 2>&1 || true; pkill -f 'vite --host 127.0.0.1 --port 4600' >/dev/null 2>&1 || true"
