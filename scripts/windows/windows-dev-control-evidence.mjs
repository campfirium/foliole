const EVIDENCE_PREFIX = 'C:/dev/foliole-android-lab-preview/.tmp/artifacts/windows-dev-action/';
...
export function parseWindowsDevLiveEvidence(output) {
  ...
  const expected = `${EVIDENCE_PREFIX}${match[1]}/a5-live.png`;
...
export function parseWindowsDevCaptureAnnotationEvidence(output) {
  ...
  const remoteRoot = `${EVIDENCE_PREFIX}${match[1]}`;
...
export function parseWindowsDevStatusEvidence(output, expectedStatus) {
  ...
  const buildIdentity = normalized.slice(EVIDENCE_PREFIX.length, -suffix.length);
  if (!normalized.startsWith(EVIDENCE_PREFIX) || !normalized.endsWith(suffix)
