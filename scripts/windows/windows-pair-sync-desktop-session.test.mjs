...
import {
  openPairSyncDesktopSession, pairSyncAuthorizationFingerprint, waitForUniquePairRequest
    ...
    evaluate: vi.fn(async (_callback, input) => {
      ...
      return {
        ...
        sync_group: { local_host_name: 'Desktop', members: [{
          authorization_id: 'desktop-authorization-1', host_name: 'Desktop', state: 'active'
        }] }
      };
    }),
...
it('launches the real current-library runtime and invokes only existing product commands', async () => {
  ...
  expect(fixture.calls.at(-1).commandArgs).toEqual({
    membership_action: 'recover_existing_member', pair_request_id: 'pair-2'
  });
  expect(session.sanitize(await session.load()).localAuthorizationFingerprint)
    .toBe(pairSyncAuthorizationFingerprint('desktop-authorization-1'));
});
...
it('approves only one request from the expected Host', async () => {
  ...
  await expect(waitForUniquePairRequest(
    session, request.host_name, { wait: vi.fn() }
  )).resolves.toBe(request);
  await expect(waitForUniquePairRequest(
    session, 'Other Host', { wait: vi.fn() }
...
it('observes a unique fixed request after the legacy window but before the shared deadline', async () => {
  ...
  await expect(waitForUniquePairRequest(session, request.host_name, {
...
it('stops request observation at the shared cancellation boundary', async () => {
  ...
  await expect(waitForUniquePairRequest(session, 'A5', {
