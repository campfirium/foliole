...
export function resolveWindowsNativePaths(repoRoot = WINDOWS_NATIVE_REPO_ROOT) {
  ...
  return {
    ...
    bootEventLogFile: joinHostPath(repoRoot, 'logs', 'windows', 'native-boot-events.ndjson'),
    ...
    nativeTaskInstallScript: joinHostPath(repoRoot, 'scripts', 'windows', 'install-native-client-interactive-task.ps1'),
    nativeTaskStateRoot: joinHostPath(repoRoot, '.tmp', 'windows-native-client-interactive'),
    nativeTaskWorkerScript: joinHostPath(repoRoot, 'scripts', 'windows', 'windows-client-native-interactive-worker.mjs'),
