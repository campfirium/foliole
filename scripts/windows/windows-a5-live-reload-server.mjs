    ...
    transformIndexHtml: {
      ...
      handler(html) {
        const script = [
          ...
          'const ready=()=>{const node=target();if(!node)return false;const rect=node.getBoundingClientRect();',
          'return rect.width>0&&rect.height>0&&rect.bottom>0&&rect.top<innerHeight;};',
          `const report=()=>fetch('${LOADED_PATH}?identity='+encodeURIComponent(identity),`,
          "{cache:'no-store'}).catch(()=>{});",
          'let settling=false;const finish=()=>{if(settling)return;settling=true;setTimeout(()=>{settling=false;',
          'if(!ready()){advance();return;}if(observer)observer.disconnect();',
          'requestAnimationFrame(()=>requestAnimationFrame(report));},500);};',
