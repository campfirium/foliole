...
describe('hosted-quality repair handoff', () => {
  it('keeps the handoff prompt to stable locator data and invokes the dev T6 controller', () => {
    const template = fs.readFileSync(templatePath, 'utf8');

    expect(template).toContain('Use `$foliole-hosted-quality-repair`');
    expect(template).toContain('failed independent top-level dev T6 run');
    ...
    expect(template).toContain('Never treat a later scheduled T6 as repair evidence');
  ...
  it('classifies T6 admission failures without making the monitor guess the stage', () => {
    ...
    expect(skill).toContain('Only an independent `.github/workflows/t6-hosted-quality.yml` run on `dev` is in repair scope');
