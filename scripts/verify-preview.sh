...
DEFAULT_VALIDATE_COMMAND="npm run quality:fast"
...
main() {
  ...
  local validate_command finish_command
  validate_command="$(resolve_command "${VERIFY_PREVIEW_VALIDATE_COMMAND:-}" "${DEFAULT_VALIDATE_COMMAND}")"
