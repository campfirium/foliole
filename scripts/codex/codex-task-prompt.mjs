...
function buildConstraintLines() {
  return [
    ...
    '- When desktop lint and Windows preview are both required on a Windows checkout, run npm run validate:desktop:serial and keep that command alive until it exits instead of launching lint:desktop and a Windows preview separately.',
    '- If a Windows preview is required, a normal final reply is only allowed after npm run windows:preview:native reaches [windows-preview] status: STARTED.',
