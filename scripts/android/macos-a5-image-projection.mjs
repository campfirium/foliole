...
export function imageProjectionMode(env) {
  ...
  if (!['', 'inspect', 'remove'].includes(mode)) throw new Error('Unknown S203 image projection mode.');
  if (mode === 'remove' && env.FOLIOLE_A5_TEST_DATA_DISPOSABLE !== '1') {
...
export async function runImageProjection(args, root, mode) {
  ...
  const result = await args.execute(args.paths.adb, ['-s', args.serial, 'shell', 'am', 'instrument', '-w', '-r',
    '-e', 'class', 'com.foliole.android.FolioleArticleImageProjectionTest',
