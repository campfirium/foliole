...
const MUTATION_ASSET_KEYS = path.join(REPO_ROOT, 'android/app/src/main/java/com/foliole/android/FolioleCompanionMutationAssetKeys.java');
...
describe('Android runtime mutation rules', () => {
  it('generates runtime mutation metadata', async () => {
    ...
    expect(definitions.runtimeMutations.groupKeys).toEqual({
      syncPushAck: 'syncPushAck',
      syncState: 'syncState'
    });
  ...
  it('keeps runtime mutation adapter wired to generated mutation rules', async () => {
    const assetKeysSource = await readFile(MUTATION_ASSET_KEYS, 'utf8');
    const storeSource = await readFile(MUTATION_STORE, 'utf8');
    const rulesSource = await readFile(RUNTIME_RULES, 'utf8');

    expect(assetKeysSource).toContain('assetKey(payload, key)');
    expect(assetKeysSource).toContain('groupKey(rules, groupName)');
    ...
    expect(rulesSource).toContain('FolioleCompanionMutationAssetKeys.ruleGroup(context, "runtimeMutations", groupName)');
