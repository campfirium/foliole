...
import {
  ...
  formatCleanupReport,
  parseArgs
} from './android-sync-cleanup-host-private.mjs';

let tempDir = '';

describe('android host-private sync cleanup', () => {
  afterEach(async () => {
  ...
  it('reports residue without deleting by default', async () => {
    ...
    const report = cleanupHostPrivateResidue(dbPath);

    expect(report.applied).toBe(false);
    expect(report.nodeViewState).toHaveLength(1);
    expect(report.nodeReadingHostState).toHaveLength(1);
    expect(report.viewStateSyncObjects).toHaveLength(1);
  ...
  it('requires destructive apply before deleting non-local host-private rows', async () => {
    const dbPath = await createSeededDatabase();

    const readOnly = cleanupHostPrivateResidue(dbPath, { apply: true, destructive: false });
    expect(readOnly.applied).toBe(false);
    expect(countRows(dbPath, 'node_reading_host_state')).toBe(2);

    const applied = cleanupHostPrivateResidue(dbPath, { apply: true, destructive: true });
    expect(applied.applied).toBe(true);
    expect(applied.nodeViewState).toHaveLength(0);
    expect(applied.nodeReadingHostState).toHaveLength(0);
    ...
    expect(countRows(dbPath, 'node_reading_host_state')).toBe(1);
    expect(countRows(dbPath, 'sync_object_state')).toBe(1);
    expect(countRows(dbPath, 'sync_delivery_receipts')).toBe(1);
  }, 30_000);
...
async function createSeededDatabase() {
  tempDir = await mkdtemp(path.join(os.tmpdir(), 'android-host-private-cleanup-'));
  ...
  db.exec(`
    ...
    CREATE TABLE node_reading_host_state (node_id TEXT, host_name TEXT, reading_position INTEGER);
  ...
  `);
  db.prepare('INSERT INTO companion_meta VALUES (?, ?)').run('host_name', 'android-local');
  ...
  db.prepare('INSERT INTO node_reading_host_state VALUES (?, ?, ?)').run('node-1', 'android-local', 10);
  db.prepare('INSERT INTO node_reading_host_state VALUES (?, ?, ?)').run('node-2', 'other-device', 20);
  ...
  db.prepare('INSERT INTO sync_delivery_receipts VALUES (?, ?)')
    .run('view_state', 'session_resume:android:phone:other-device:active_node');
  db.close();
  return dbPath;
}
