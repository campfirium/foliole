...
const SCENARIO = 'sync-group-migration';
...
const ACCEPTANCE_ENV_KEYS = [
  'VITE_FOLIOLE_IOS_BRIDGE_ACCEPTANCE',
  'VITE_FOLIOLE_IOS_BRIDGE_ACCEPTANCE_ENDPOINT',
  'VITE_FOLIOLE_IOS_BRIDGE_ACCEPTANCE_SCENARIO'
];

export async function runIosSyncGroupMigrationAcceptance(repoRoot, artifactDir) {
  mkdirSync(artifactDir, { recursive: true });
  const options = {
    artifactDir,
    derivedData: path.join(artifactDir, 'DerivedData'),
    repoRoot,
    resourceMode: resolveIosResourceMode()
  };
  ...
  try {
    ...
    const container = capture(options, 'xcrun', ['simctl', 'get_app_container', owned.udid, BUNDLE_ID, 'data']).trim();
    ...
    const result = await waitForAcceptanceObservation({
      ...
      action: () => run(options, 'xcrun', ['simctl', 'launch', '--terminate-running-process', owned.udid, BUNDLE_ID]),
      describe: (value) => `sync-group migration status=${value?.status ?? 'missing'}`,
      initialObservation: 'sync-group migration result was not readable',
      label: 'signed iOS Sync Group migration acceptance',
      read: () => JSON.parse(readFileSync(resultPath, 'utf8')),
      timeoutMs: 180_000
    });
    verifyResult(result);
    const receipt = { accepted_tip: revision, result, signature_identifier: signature, simulator: owned };
    ...
    try { restoreOrdinaryAssets(options); } finally {
      if (owned) cleanupOwnedSimulator(options, owned.udid);
    }
  }
}
...
function prepareApp(options, udid) {
  ...
  const args = createSimulatorAcceptanceBuildArgs({
    ...
    resourceArgs: iosXcodebuildResourceArgs(options.resourceMode),
    udid
  });
...
function assertFrozenRevision(options) {
  const status = capture(options, 'git', ['status', '--porcelain', '--untracked-files=no']).trim();
  if (status) throw new Error('iOS Sync Group migration acceptance requires a clean tracked worktree.');
  const revision = capture(options, 'git', ['rev-parse', 'HEAD']).trim();
  const remote = capture(options, 'git', ['rev-parse', 'origin/dev']).trim();
  if (!revision || revision !== remote) throw new Error('iOS Sync Group migration acceptance requires HEAD == origin/dev.');
  return revision;
}

function verifyResult(result) {
  if (result?.status !== 'passed' || result?.scenario !== SCENARIO || result?.phase !== 'migration-verified') {
    throw new Error(result?.error || 'iOS Sync Group migration acceptance evidence is incomplete.');
  }
  const legs = Array.isArray(result.legs) ? result.legs : [];
  if (legs.length !== 4 || typeof legs[0]?.decision_digest !== 'string') {
    throw new Error('iOS Sync Group migration acceptance receipt is incomplete.');
  }
}

function cleanupOwnedSimulator(options, udid) {
  cleanupOwnedIosSimulator({
    artifactDir: options.artifactDir,
    bundleId: BUNDLE_ID,
    captureLog: (args) => captureAllowFailure(options, 'xcrun', args),
    runAllowFailure: (args) => runAllowFailure(options, 'xcrun', args),
    udid
  });
}
