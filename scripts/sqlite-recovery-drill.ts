...
function printUsage() {
  console.error('usage: node scripts/electron-sqlite-runner.mjs scripts/sqlite-recovery-drill.ts [--work-dir <dir>] [--source-path <test.db>] [--backup-path <new.db>] [--restore-path <new.db>]');
}

if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve('scripts/sqlite-recovery-drill.ts')) {
