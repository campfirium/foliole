...
const JoinDialogContent = forwardRef<HTMLDivElement, {
  ...
  return (
    <AppDialogContent aria-describedby={undefined} className="w-[min(460px,calc(100vw-48px))]" layout="task"
      ...
      <AppDialogBody className="space-y-5 text-center">
      ...
      <AppDialogActions>
        <AppButton disabled={disabled} onClick={() => void run(state.rejectRequest)} variant="danger">
          {t('settings.companionSync.group.join.reject')}
        </AppButton>
        <AppButton disabled={disabled} loading={disabled}
