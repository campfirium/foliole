...
function InteractionHarness() {
  return (
    <LocalizationProvider initialLanguagePreference="en">
      <PdfVisualExcerptRuntimeProvider currentPage={1} locators={[{
        id: 'anchor-1', kind: 'image-excerpt', label: 'Excerpt', nodeId: 'excerpt-1', page: 1,
        rects: [{ height: 0.4, width: 0.4, x: 0.2, y: 0.2 }], x: 0.2, y: 0.2
      }]} nodeId="pdf-1" rotation={0} source="fixture.pdf">
        <div data-testid="page-root">
          <canvas data-testid="pdf-canvas" />
          <div className="textLayer"><span data-testid="pdf-text">Words</span><span className="endOfContent" data-testid="pdf-end" /></div>
...
describe('PDF visual excerpt page interaction', () => {
  ...
  it('keeps text spans native and treats the auxiliary text-layer element as visual area', async () => {
    ...
    dispatchPointer(screen.getByTestId('pdf-text'), 'pointerdown', { clientX: 70, clientY: 10, pointerId: 2 });
    dispatchPointer(screen.getByTestId('pdf-text'), 'pointerup', { clientX: 90, clientY: 30, pointerId: 2 });
    expect(screen.queryByTestId('pdf-image-excerpt-error')).not.toBeInTheDocument();

    dispatchPointer(screen.getByTestId('pdf-end'), 'pointerdown', { clientX: 70, clientY: 10, pointerId: 3 });
    dispatchPointer(screen.getByTestId('pdf-end'), 'pointerup', { clientX: 90, clientY: 30, pointerId: 3 });
...
it('renders the toolbar excerpt affordance as a non-button status icon', () => {
  render(<LocalizationProvider initialLanguagePreference="en"><PdfVisualExcerptToolbarControls onToolbarInteraction={vi.fn()} /></LocalizationProvider>);
