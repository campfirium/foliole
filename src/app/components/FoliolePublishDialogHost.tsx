    ...
    <AppDialog open onOpenChange={(open) => !open && busy === 'idle' && setRequest(null)}>
      <AppDialogPortal><AppDialogOverlay /><AppDialogContent aria-describedby={undefined} className="w-[min(960px,calc(100vw-32px))] p-6" onKeyDownCapture={keyDown}>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <AppButton disabled={busy !== 'idle'} onClick={() => void run('preview')} variant="subtle">{t(busy === 'previewing' ? 'desktop.foliolePublish.previewing' : 'desktop.foliolePublish.preview')}</AppButton>
          <AppButton disabled={!configured || busy !== 'idle'} onClick={() => void run('publish')}>{t(busy === 'publishing' ? 'desktop.foliolePublish.publishing' : 'desktop.foliolePublish.publish')}</AppButton>
        </div>
