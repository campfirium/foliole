...
const TITLEBAR_ACTION_GAP = WINDOW_TITLEBAR_RIGHT_PANEL_GAP;
...
export function resolveVisibleRightPanelCount(args: {
  ...
  if (remainingWidth < TITLEBAR_ACTION_WIDTH + TITLEBAR_ACTION_GAP) {
    return 0;
  }
  const count = Math.floor(remainingWidth / (TITLEBAR_ACTION_WIDTH + TITLEBAR_ACTION_GAP));
