...
function DeferredTargetStatus({ searchStatus, searchTarget }: { searchStatus: { current: number; total: number }; searchTarget: { id: number } | null }) {
  return <p data-testid="pdf-search-deferred-target-status">{`${searchStatus.current}/${searchStatus.total}/${searchTarget ? 'pending' : 'cleared'}`}</p>;
}

function DeferredTargetPage({
  pageElementsRef,
  textReady
}: {
  pageElementsRef: MutableRefObject<Record<number, HTMLDivElement | null>>;
...
function PdfSearchDeferredTargetScrollHarness() {
  const scrollContainerRef = useRef<HTMLDivElement | null>(null);
  const pageElementsRef = useRef<Record<number, HTMLDivElement | null>>({});
  const pageTextByNumberRef = useRef<Record<number, string>>({
    1: 'first match entry',
  ...
  return (
    <div ref={scrollContainerRef}>
      ...
      <DeferredTargetPage pageElementsRef={pageElementsRef} textReady={textReady} />
      <button
        onClick={() => {
          setTextReady(true);
      ...
      <DeferredTargetStatus searchStatus={searchStatus} searchTarget={searchTarget} />
    </div>
  );
}
...
describe('usePdfSearchEffect target jump', () => {
  ...
  it('waits for the text layer geometry before consuming an external target jump', async () => {
    const scrollToMock = vi.fn();
    window.HTMLElement.prototype.scrollTo = scrollToMock;

    render(<PdfSearchDeferredTargetScrollHarness />);

    await waitFor(() => {
      expect(screen.getByTestId('pdf-search-deferred-target-status')).toHaveTextContent('1/1/pending');
    });
    expect(scrollToMock).not.toHaveBeenCalled();

    fireEvent.click(screen.getByRole('button', { name: 'reveal-text-layer' }));

    await waitFor(() => {
      expect(screen.getByTestId('pdf-search-deferred-target-status')).toHaveTextContent('1/1/cleared');
    });
    expect(scrollToMock).toHaveBeenCalledTimes(1);
  });
});
