...
import type { LinkPanelPosition, LinkPanelViewportBounds } from './linkPanelViewport';

export interface ExternalDocumentPreviewPanelSize {
  height: number;
  width: number;
}
...
export function normalizeExternalDocumentPreviewPanelSize(
  size: ExternalDocumentPreviewPanelSize
...
function getDefaultExternalDocumentPreviewPanelSize() {
  return normalizeExternalDocumentPreviewPanelSize({
    height: Math.floor(getViewportHeight() * DEFAULT_HEIGHT_RATIO),
    width: Math.floor(getViewportWidth() * DEFAULT_WIDTH_RATIO)
  });
}

export function loadExternalDocumentPreviewPanelSize() {
  const raw = getWhitelistedLocalStorageItem(APP_SETTINGS_STORAGE_KEYS.externalDocumentPreviewPanelSize);
  if (!raw) {
    return getDefaultExternalDocumentPreviewPanelSize();
  }
  try {
    const parsed = JSON.parse(raw) as Partial<ExternalDocumentPreviewPanelSize>;
...
export function saveExternalDocumentPreviewPanelSize(size: ExternalDocumentPreviewPanelSize) {
  setWhitelistedLocalStorageItem(
    APP_SETTINGS_STORAGE_KEYS.externalDocumentPreviewPanelSize,
    JSON.stringify(normalizeExternalDocumentPreviewPanelSize(size))
  );
}

export function buildExternalDocumentPreviewFullscreenStyle(bounds: LinkPanelViewportBounds) {
  return {
    height: `${Math.max(280, bounds.windowHeight - VIEWPORT_MARGIN * 2)}px`,
    left: `${VIEWPORT_MARGIN}px`,
    top: `${VIEWPORT_MARGIN}px`,
    width: `${Math.max(320, bounds.windowWidth - VIEWPORT_MARGIN * 2)}px`
  };
}

export function buildExternalDocumentPreviewWindowedStyle(
