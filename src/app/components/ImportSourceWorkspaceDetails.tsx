    ...
    <AppDialog onOpenChange={onOpenChange} open={open}>
      <AppDialogPortal>
        ...
        <AppDialogContent
          ...
          className="left-1/2 top-1/2 h-[min(760px,calc(100vh-120px))] w-[min(1360px,calc(100vw-180px))] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-border/80 bg-bg-panel p-0"
        >
          <section aria-label="Import management" className="flex h-full min-h-0 flex-col">
            <header className="flex items-center justify-between px-6 pb-3 pt-5">
              ...
              <AppButton aria-label="Close import management" className="size-8 px-0" onClick={() => onOpenChange(false)} variant="ghost">
