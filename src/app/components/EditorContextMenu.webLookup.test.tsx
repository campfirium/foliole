...
it('keeps removed built-in items out of the right-click menu', () => {
  window.localStorage.setItem(APP_SETTINGS_STORAGE_KEYS.editorContextMenuOrder, JSON.stringify(['lookup:chatgpt', 'lookup:google']));
  const payload = createSelectionPayload();
  ...
  />);
  expect(screen.getAllByRole('menuitem').map((item) => item.textContent)).toEqual(['Chat with ChatGPT', 'Search with Google']);
});
