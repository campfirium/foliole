...
it('pins worker to packaged legacy pdfjs worker with explicit version and clears stale worker global', async () => {
  (globalThis as Record<string, unknown>).pdfjsWorker = { stale: true };

  vi.resetModules();
  await import('./PdfDocumentSurface');

  expect(mockPdfWorkerOptions.workerSrc).toContain('pdfjs-dist/legacy/build/pdf.worker.min.mjs');
