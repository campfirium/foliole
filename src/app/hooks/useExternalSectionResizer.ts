...
import {
  loadExternalSectionHeightPreference,
  saveExternalSectionHeightPreference
} from '../../store/workspaceLayoutPrefs';

const MAX_HEIGHT = 360;
const MIN_HEIGHT = 120;
const STEP = 16;
export const EXTERNAL_SECTION_HEIGHT_DEFAULT = 180;
...
export function useExternalSectionResizer(initialHeight = EXTERNAL_SECTION_HEIGHT_DEFAULT) {
  const [height, setHeight] = useState(() => clampHeight(loadExternalSectionHeightPreference(initialHeight)));
  const [isResizing, setIsResizing] = useState(false);
  const resizeStartRef = useRef<ResizeStartState | null>(null);

  useEffect(() => {
    saveExternalSectionHeightPreference(height);
  }, [height]);
