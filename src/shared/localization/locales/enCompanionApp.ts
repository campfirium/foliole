export const EN_COMPANION_APP_TRANSLATIONS = {
  'companion.link.unavailable': 'This link cannot be opened in this library.',
  'companion.link.dismiss': 'Dismiss',
  'companion.app.retry': 'Retry',
  'companion.app.starting.eyebrow': 'Companion runtime',
  'companion.app.starting.message':
    'Preparing a stable device identity and local companion storage before the topic surface loads.',
  'companion.app.starting.title': 'Starting companion runtime',
  'companion.app.bootstrap.module': 'Companion bootstrap',
  'companion.app.bootstrapFailed': 'Companion bootstrap failed',
  'companion.app.iosPrepared.eyebrow': 'Foliole for iPhone',
  'companion.app.iosPrepared.message':
    'Local storage is ready. Topic browsing, review, and sync are not available in this build.',
  'companion.app.iosPrepared.title': 'This iPhone is ready'
} as const;
