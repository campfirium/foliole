...
import { systemEntryDisplayNameOverride } from './systemEntryDisplayNamesStore';
...
const SYSTEM_ENTRY_NAMES: Record<AppLocale, Record<SystemEntryId, string>> = {
  ...
  es: {
    ...
    trash: 'Papelera',
    'virtual-root': 'Carpetas virtuales',
  ...
  fr: {
    ...
    trash: 'Corbeille',
    'virtual-root': 'Dossiers virtuels',
    published: 'Publiés',
    shelved: 'En attente',
    removed: 'Retirés'
  },
  it: {
    ...
    trash: 'Cestino',
    'virtual-root': 'Cartelle virtuali',
  ...
  pl: {
    ...
    trash: 'Kosz',
    'virtual-root': 'Foldery wirtualne',
  ...
  ru: {
    ...
    inbox: 'Входящие',
    trash: 'Корзина',
    'virtual-root': 'Виртуальные папки',
...
export function resolveSystemEntryDisplayName(locale: AppLocale, id: SystemEntryId) {
  return systemEntryDisplayNameOverride(id) ?? defaultSystemEntryDisplayName(locale, id);
}
...
export function resolveNodeDisplayTitle(locale: AppLocale, nodeId: string, storedTitle: string) {
  ...
  return id ? resolveSystemEntryDisplayName(locale, id) : storedTitle;
}
