...
async function applyLocalObject(
  ...
  const deviceId = await iosCompanionDeviceId(db);
  const now = new Date().toISOString();
  ...
  await apply(record);
  await markIosCompanionMutation({ contentHash, db, deviceId, objectId, objectType, updatedAt: now });
  return { content_hash: contentHash, object_id: objectId };
}
...
async function mark(db: DbPort, objectType: string, objectId: string, contentHash: string, updatedAt: string) {
  const deviceId = await iosCompanionDeviceId(db);
  return markIosCompanionMutation({ contentHash, db, deviceId, objectId, objectType, updatedAt });
}
...
async function insertReviewLog(db: DbPort, nodeId: string, opId: string, draft: Record<string, unknown>) {
  ...
  await db.run(
    'INSERT OR IGNORE INTO review_log (id, op_id, device_id, node_id, grade, scheduler_version, reviewed_at, due_before, stability_before, difficulty_before, due_after, stability_after, difficulty_after) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [crypto.randomUUID(), opId, await iosCompanionDeviceId(db), nodeId, Number(draft.grade), String(draft.schedulerVersion), String(draft.reviewedAt), String(before.due), Number(before.stability), Number(before.difficulty), String(after.due), Number(after.stability), Number(after.difficulty)]
  );
}
