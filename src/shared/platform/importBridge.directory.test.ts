...
const DIRECTORY_IMPORT_PAYLOAD = {
...
const EXPECTED_DIRECTORY_IMPORT_RESULT = {
...
it('normalizes the directory import payload', async () => {
  const invoke = vi.fn().mockResolvedValue(DIRECTORY_IMPORT_PAYLOAD);
  window.electronAPI = createMockElectronApi(invoke);

  await expect(runRuntimeDirectoryImport()).resolves.toEqual(EXPECTED_DIRECTORY_IMPORT_RESULT);
