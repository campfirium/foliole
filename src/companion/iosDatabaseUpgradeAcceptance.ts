...
export async function runIosDatabaseUpgradeAcceptance() {
  try {
    ...
    const bootstrap = await initializeIosCompanionDatabase(native, undefined, {
      afterRepair: shouldInjectIosDatabaseUpgradeFault()
        ? (index) => { if (index === 0) throw new Error('Injected iOS database upgrade acceptance fault.'); }
        : undefined
    });
