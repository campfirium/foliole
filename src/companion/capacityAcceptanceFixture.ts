...
export async function seedCapacityNodes(db: DbPort, start: number, count: number, hostName: string) {
  ...
  await db.transaction(async (tx) => {
    for (let index = start; index < start + count; index++) await seedNode(tx, index, 4096, hostName);
  });
}
async function seedNode(db: DbPort, index: number, bytes: number, hostName: string) {
  const id = `node-${index}`;
  ...
  if (index % 5 === 0) await seedMetadata(db, id, hostName);
}
...
async function seedMetadata(db: DbPort, id: string, hostName: string) {
  ...
  await db.run('INSERT INTO node_view_state (node_id,host_name,scroll_top,updated_at) VALUES (?,?,?,?)', [id, hostName, 100, time]);
