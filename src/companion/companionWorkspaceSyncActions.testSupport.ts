...
export function createSnapshot(nodeId = 'topic-1'): WorkspaceSnapshot {
  return {
    activeNodeId: nodeId,
    nodeOrder: [nodeId],
    nodesById: {
...
export function createSyncState(
  overrides: Partial<NativeCompanionWorkspaceSyncState> = {}
...
export function createSyncResult(): CompanionDesktopSyncResult {
  return {
    appliedNodeIds: [],
    appliedObjectIds: [],
...
export function createActions() {
  ...
  return {
    actions: createWorkspaceSnapshotActions({ ...callbacks, state: createSyncState() }),
    callbacks
  };
}

export function getSyncObjectsMock() {
  return syncObjectsMock;
}

export function getWorkspaceSyncMock() {
  return workspaceSyncMock;
}

export function resetSyncActionMocks() {
  vi.resetAllMocks();
