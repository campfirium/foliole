...
function member(hostName: string, kind: string, name: string, state: 'active' | 'left'): SyncGroupMemberPayload {
  return {
    approved_by_host_name: 'Studio', authorization_id: `${hostName}-auth`, host_name: name,
    host_platform: kind, joined_at: '2026-08-08T00:00:00.000Z', state
  };
}

function renderDevicePriority(members: SyncGroupMemberPayload[]) {
  return render(<CompanionSyncGroupOverview group={{
    created_at: '2026-08-08T00:00:00.000Z', created_by_host_name: 'desktop-1', display_name: 'Maci',
    group_id: 'group-1', local_host_name: 'local-1', local_member_state: 'active', members,
...
it('keeps transient pipeline details out of the Sync Group overview', () => {
  ...
  render(
    <CompanionSyncPanel
      ...
      syncGroup={{
        ...
        created_by_host_name: 'desktop-1',
        display_name: 'Maci',
        group_id: 'group-1',
        local_host_name: 'ios-1',
        local_member_state: 'active',
        members: [
          {
            approved_by_host_name: 'Maci', authorization_id: 'desktop-auth', host_name: 'Maci',
            host_platform: 'darwin', joined_at: '2026-08-08T00:00:00.000Z', state: 'active'
          },
          {
            approved_by_host_name: 'Maci', authorization_id: 'ios-auth', host_name: 'Foliole Review',
            host_platform: 'ios-capacitor', joined_at: '2026-08-08T00:01:00.000Z', state: 'active'
          },
          {
            approved_by_host_name: 'Maci', authorization_id: 'android-auth', host_name: 'A5',
            host_platform: 'android-capacitor', joined_at: '2026-08-08T00:02:00.000Z', state: 'active'
          }
        ],
        timeline_id: 'timeline-1'
      }}
  ...
  expect(screen.queryByTestId('companion-sync-source-indicator')).not.toBeInTheDocument();
...
it('falls back to the local Host when no remote Host is active', () => {
  renderDevicePriority([
    member('desktop-1', 'darwin', 'Maci', 'left'),
  ...
  expect(screen.getByTestId('companion-sync-group-device')).toHaveTextContent(/^A5Android/);
});
