export const allowedRegexInventory = [
  ...
  { path: 'src/features/editor/adapters/liveMarkdownImageSurface.ts', line: "if (!imageMatch.attachmentId && editorNodeId && /^https?:\\/\\//u.test(imageMatch.source)) {", owner: 'remote image localization', reason: 'Limits on-demand localization to external HTTP(S) image sources.' },
