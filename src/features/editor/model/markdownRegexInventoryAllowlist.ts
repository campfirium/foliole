export const allowedRegexInventory = [
  ...
  { path: 'src/features/editor/adapters/liveMarkdownLinePlugin.ts', line: 'const MARKDOWN_DECORATION_CONTEXT_PATTERN = /[\\\\`*_{}\\[\\]()#+\\-.!|<>]/;', owner: 'live markdown parse reuse', reason: 'Detects Markdown decoration context before using mapped decoration reuse for plain text input.' },
  { path: 'src/features/editor/adapters/liveMarkdownLinePlugin.ts', line: 'if (MARKDOWN_DECORATION_CONTEXT_PATTERN.test(update.startState.doc.toString())) {', owner: 'live markdown parse reuse', reason: 'Avoids mapped decoration reuse when the current document has Markdown widgets or inline syntax.' },
  { path: 'src/features/editor/adapters/liveMarkdownLinePlugin.ts', line: 'plainTextInput = text.length > 0 && !/[\\n\\r]/.test(text);', owner: 'live markdown parse reuse', reason: 'Allows mapped decoration reuse only for single-change plain text input after Markdown context is excluded.' },
