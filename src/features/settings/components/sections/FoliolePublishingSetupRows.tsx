...
function SetupInput(props: {
  ...
  return <input aria-describedby={props.ariaDescribedBy} aria-invalid={props.invalid || undefined} aria-label={props.ariaLabel} autoComplete="off" className={settingsFieldClassName(props.className)} disabled={props.disabled} name={props.name} onBlur={props.onBlur} onChange={(event) => props.onChange(event.target.value)} onFocus={props.onFocus} onKeyDown={(event) => { if (event.key === 'Enter') props.onEnter?.(); }} placeholder={props.placeholder} spellCheck={false} type={props.type ?? 'text'} value={props.value} />;
}
...
function PublishInstructionStep() {
  const t = useTranslation();
  return <PublishingSetupStep description={t('settings.publishing.foliole.publish.description')} title={t('settings.publishing.foliole.publish.title')} />;
}
...
export function FoliolePublishingSetupRows({ state }: { state: FoliolePublishingSettingsState }) {
  ...
  return <>
    ...
    >
      <CredentialInput ariaLabel={t('settings.publishing.foliole.token.aria')} disabled={state.connected || state.disabled} error={state.apiTokenInvalid ? t('settings.publishing.foliole.token.invalid') : undefined} name="foliole-publish-api-token" onBlur={() => { setEditingSavedToken(false); state.saveDraft(); }} onChange={(apiToken) => state.updateForm({ apiToken })} onEnter={() => { setEditingSavedToken(false); state.saveDraft(); }} onFocus={() => { if (state.hasSavedToken && !state.form.apiToken) setEditingSavedToken(true); }} placeholder={t('settings.publishing.foliole.token.placeholder')} type="password" value={tokenValue} />
    </PublishingSetupStep>
    <PublishingSetupStep
    ...
    >
      <CredentialInput ariaLabel={t('settings.publishing.foliole.account.aria')} disabled={state.connected || state.disabled} error={state.accountIdInvalid ? t('settings.publishing.foliole.account.invalid') : undefined} name="foliole-publish-account-id" onBlur={state.saveDraft} onChange={(accountId) => state.updateForm({ accountId })} onEnter={state.saveDraft} placeholder={t('settings.publishing.foliole.account.placeholder')} value={state.form.accountId} />
    </PublishingSetupStep>
    <PublishingSetupStep description={t(state.connected ? 'settings.publishing.foliole.project.connectedDescription' : 'settings.publishing.foliole.project.description')} title={t('settings.publishing.foliole.project.title')}>
      {state.connected ? <>
        <span className={settingsReadOnlyFieldClassName('min-w-0 flex-1 truncate')}>{state.pagesUrl}</span>
