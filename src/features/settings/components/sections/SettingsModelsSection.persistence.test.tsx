...
beforeEach(() => {
  ...
  assistantRuntime.loadAssistantModelSettings.mockResolvedValue({
    models: [{
      api_key_length: 37, endpoint: 'https://models.example/v1/chat/completions',
      has_api_key: true, id: 'model-a', model: 'model-a', state: 'configured'
    }],
...
it('saves a custom model draft before testing it', async () => {
  assistantRuntime.saveAssistantModelDraft.mockImplementation(async (input) => ({
    models: [{
      api_key_length: input.api_key?.length ?? 0,
  ...
  assistantRuntime.testAssistantModel.mockResolvedValue({
    settings: {
      models: [{
        ...
        api_key_length: 6, endpoint: 'https://second.example/v1/chat/completions',
        has_api_key: true, id: 'model-b', model: 'model-b', state: 'configured'
      }],
      selected_model_id: 'codex'
    },
    state: 'ready'
  ...
  await waitFor(() => expect(assistantRuntime.saveAssistantModelDraft).toHaveBeenLastCalledWith(
    expect.objectContaining({ api_key: 'secret', model: 'model-b' })
  ));
...
it('starts with one empty custom choice when no model has been saved', async () => {
  assistantRuntime.loadAssistantModelSettings.mockResolvedValue({
    models: [], selected_model_id: 'codex'
  });
  renderWithLocalization(<SettingsModelsSection />);

  expect(await screen.findByRole('radio', { name: 'Use Custom model' })).toBeDisabled();
...
it('restores an untested draft quietly without adding another row', async () => {
  assistantRuntime.loadAssistantModelSettings.mockResolvedValue({
    models: [{
      api_key_length: 21, endpoint: 'https://draft.example/v1', has_api_key: true, id: 'draft-a',
...
it('keeps a failed model in the form while leaving it unavailable for selection', async () => {
  ...
  assistantRuntime.testAssistantModel.mockResolvedValue({
    ...
    settings: {
      models: [{
        api_key_length: 7, endpoint: 'https://failed.example/v1/chat/completions',
        has_api_key: true, id: 'failed-model', model: 'model-failed', state: 'not_configured'
      }],
      selected_model_id: 'codex'
    },
    state: 'failed'
...
it('shows the provider failure message without replacing it with a local guess', async () => {
  const providerMessage = 'User location is not supported for the API use.';
  assistantRuntime.testAssistantModel.mockResolvedValue({
    failure: { category: 'provider_region_unsupported', message: providerMessage },
    settings: {
      models: [{
        api_key_length: 39, endpoint: 'https://models.example/v1/chat/completions',
        has_api_key: true, id: 'model-a', model: 'model-a', state: 'not_configured'
      }],
      selected_model_id: 'codex'
    },
    state: 'failed'
  ...
  expect(await screen.findByText(providerMessage)).toBeInTheDocument();
  expect(screen.queryByText(/tool round trip/iu)).not.toBeInTheDocument();
});

it('refreshes the exact stored key mask after a persisted model test fails', async () => {
  assistantRuntime.loadAssistantModelSettings.mockResolvedValue({
    models: [{
      api_key_length: 0, endpoint: 'https://failed.example/v1/chat/completions',
      has_api_key: true, id: 'failed-model', model: 'model-failed', state: 'not_configured'
    }],
    selected_model_id: 'codex'
  });
  assistantRuntime.testAssistantModel.mockResolvedValue({
    failure: { category: 'tools_unsupported' },
    settings: {
      models: [{
        api_key_length: 39, endpoint: 'https://failed.example/v1/chat/completions',
        has_api_key: true, id: 'failed-model', model: 'model-failed', state: 'not_configured'
      }],
      selected_model_id: 'codex'
    },
  ...
  await screen.findByDisplayValue('model-failed');
  expect(screen.getByLabelText('API key')).not.toHaveAttribute('placeholder');
  fireEvent.click(screen.getByRole('button', { name: 'Test' }));

  await waitFor(() => expect(assistantRuntime.testAssistantModel).toHaveBeenCalled());
  expect(screen.getByLabelText('API key')).toHaveAttribute('placeholder', '•'.repeat(39));
  expect(screen.getAllByLabelText('Model')).toHaveLength(1);
  expect(screen.queryByPlaceholderText('Custom model')).not.toBeInTheDocument();
});
