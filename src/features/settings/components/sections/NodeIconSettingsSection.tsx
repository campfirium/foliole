...
function StateStyleRow(props: {
  ...
  return (
    <SettingsRow
      ...
      description="Choose line type and color for this icon state. Dashed states can tune dash and gap lengths."
      title={props.label}
    >
      <SettingsControlSlot className="flex-[0_0_360px] flex-col items-stretch">
...
function PreviewRow(props: {
  ...
  return (
    <SettingsRow
      className="items-start"
      description="Topic and review item SVGs keep type semantics. Pending, scheduled, and dismissed styling comes from state settings."
      title="Preview"
    >
      <SettingsControlSlot className="flex-[0_0_360px] flex-col items-stretch">
        <div aria-label="Icon preview" className="flex flex-wrap items-center gap-2.5">
          <PreviewIcon kind="reading" label="Topic pending" state="pending" />
          <PreviewIcon kind="review" label="Review item pending" state="pending" />
          <PreviewIcon kind="reading" label="Topic scheduled" state="scheduled" />
          <PreviewIcon kind="review" label="Review item scheduled" state="scheduled" />
          <PreviewIcon
...
export function NodeIconSettingsSection() {
  ...
  return (
    <SettingsSection ariaLabel="Navigation icon settings section" title="Navigation icons">
      <SvgRow
        description="Paste the SVG used for topic rows. Leave empty to fall back to the built-in geometry icon."
        label="Topic SVG"
        onChange={state.setTopicSvg}
        value={state.topicSvg}
      />
      <SvgRow
        description="Paste the SVG used for review item rows. Leave empty to reuse the topic SVG fallback behavior."
        label="Review item SVG"
