  ...
  return (
    <div className="min-w-full" style={{ width: props.pageWidth, paddingBottom: 'calc(100dvh - 12rem)' }}>
      <VirtualListSurface key={props.pageWidth} items={pages} getItemKey={pageKey} estimateSize={estimateSize}
        ...
        renderItem={(page) => <WindowPage crops={crops} page={page} width={props.pageWidth} focused={props.initialPage === page} onLayoutReady={onLayoutReady} />} />
    </div>
  );
}

function WindowPage(props: { crops: Map<number, PageLayout>; page: number; width: number; focused: boolean; onLayoutReady(page: number): void }) {
  ...
  useLayoutEffect(() => {
    if (measured && rowRef.current) props.crops.set(props.page, { cropBox, height: rowRef.current.getBoundingClientRect().height });
  }, [cropBox, measured, props.crops, props.focused, props.page]);
