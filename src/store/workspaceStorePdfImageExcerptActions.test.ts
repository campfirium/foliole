...
function confirmCreation(args: {
  ...
  return {
    ...
    nodes: [{
      ...
      reveal: null, anchorLink: args.node.anchorLink ?? null, imageRegions: null, position: args.position,
...
it('numbers excerpts per source without reusing deleted numbers or overwriting manual titles', async () => {
  ...
  harness.setState({
    nodesById: { ...harness.getState().nodesById, [second]: { ...harness.getState().nodesById[second]!, title: 'Diagram' } },
