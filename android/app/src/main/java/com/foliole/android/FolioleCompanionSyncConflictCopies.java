...
final class FolioleCompanionSyncConflictCopies {
    ...
    static String create(SQLiteDatabase database, JSONObject record, JSONObject snapshot, String deviceId, String now) throws Exception {
        ...
        String copyNodeId = conflictCopyNodeId(versionId);
        if (nodeExists(database, copyNodeId)) {
        ...
        String localVersionId = deviceId + "#" + copyNodeId;
    ...
    private static String conflictCopyNodeId(String versionId) throws Exception {
        return "conflict-copy-" + sha256(versionId.getBytes(StandardCharsets.UTF_8)).substring(0, 32);
    }
    ...
    private static String conflictCopyTitle(JSONObject snapshot, String sourceDeviceId) {
        String title = snapshot.optString("title", "Untitled").trim();
        if (title.isEmpty()) {
            title = "Untitled";
        }
