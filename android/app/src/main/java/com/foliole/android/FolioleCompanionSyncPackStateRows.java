...
final class FolioleCompanionSyncPackStateRows {
    ...
    private static void insertCleanStateRow(
        ...
        insertState.bindString(4, cursor.getString(2));
        insertState.bindString(5, deviceId);
        insertState.bindString(6, cursor.getString(3));
        if (cursor.isNull(4)) {
            insertState.bindNull(7);
        } else {
            insertState.bindString(7, cursor.getString(4));
        }
