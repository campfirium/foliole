...
- Mac 本地 A5 日常开发动作直接消费当前工作区源码，不要求先提交或 push。动作承担闭环最终宿主验收时，先提交候选并使用 `node scripts/android/macos-a5-dev.mjs <action> --formal`；该入口要求动作前后均为干净 `dev`、`HEAD` 不变并回执唯一 accepted tip，期间工作区变化即使本次验收失效。Windows 跨宿主动作继续使用普通本地 `dev` Git 仓库：Mac controller 将当前工作区 `dev` 精确覆盖到 LAN Git 的同名传输镜像，Windows 只在工作区干净时精确跟随该镜像并运行固定 adapter；Windows 不提交或推送源码上游。
