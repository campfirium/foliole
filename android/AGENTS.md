...
- Mac 本地 A5 动作直接消费当前工作区源码，不要求先提交或 push；正式验收证据必须记录当前工作区状态并拒绝把未提交源码伪装成可复现版本。Windows 跨宿主动作继续使用普通本地 `dev` Git 仓库：Mac controller 将当前工作区 `dev` 精确覆盖到 LAN Git 的同名传输镜像，Windows 只在工作区干净时精确跟随该镜像并运行固定 adapter；Windows 不提交或推送源码上游。
