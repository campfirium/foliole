...
- Mac 是 Android 日常开发主机：源码、companion 构建、Capacitor sync、Gradle、固定 A5 的 ADB 安装 / 启动 / 日志与 instrumentation 必须在同一 Mac 工作区闭环，并优先使用 `node scripts/android/macos-a5-dev.mjs <status|build|capture-annotation|deploy>`。该入口只接受内建 A5 serial，不卸载主应用、不清数据、不改配对、不扫描或猜测设备，并在动作结束后停止本轮 ADB server；Android Studio 与模拟器不进入日常路径。
- Windows 保留为 Foliole desktop 当前库、真实 LAN 同步对象和 Windows / 跨宿主最终验收端。只有验收目标本身依赖 Windows desktop runtime、当前 Windows library、Windows 专属行为或用户明确要求 Windows A5 验收时，才通过 `scripts/windows/windows-dev-control.mjs` 的固定动作进入 Windows adapter；不得把 Windows controller 作为普通 Android UI / WebView 调试的默认入口。
