...
- 实体 Windows Android Lab 使用普通本地 Git 仓库作为 A5 开发现场；Mac 推送当前已提交 `dev` 后，Windows 只做 fast-forward pull，再运行既有 A5 开发动作。Windows 不得提交 / 推送源码上游；Lab 仍使用独立 key、task、状态和证据根，禁止开放任意远程 shell。
