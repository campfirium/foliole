...
- Mac A5 日常动作必须机械分流：`status` 只读取固定设备、应用与 workspace readiness；`build` 完成 companion web build、Capacitor sync 与无 daemon debug / androidTest APK 构建；`deploy` 先执行同一 build，再用 `adb install -r` 保留数据覆盖安装、冷启动并复核 Activity，普通与 formal `deploy` 均不以前置数据库备份或状态比对为条件。任一阶段失败必须按原阶段失败，不得卸载、清数据、使用 install cache 掩盖 stale assets 或隐式回退其他宿主。
...
- Mac fixed adapter 只调用具名、显式 serial 的 purpose-specific helpers；日常使用固定入口，不口头推荐散落的裸 Gradle、ADB 或 Capacitor 命令。`deploy` 成功必须证明覆盖安装与目标 Activity 启动；数据完整性与 workspace readiness 验收由明确需要该结果的独立场景承担。
