...
- 实体 Windows 开发机是 Android 原生宿主与 A5 设备操作的唯一执行端；Mac 只允许编辑源码、运行不启动 Android 宿主的静态 / TypeScript 测试，以及通过 `scripts/windows/windows-dev-control.mjs` 执行普通 `dev` push 并发起固定动作。设备 mutation 只走固定 device adapter；adapter 尚未收口时停止，不恢复旧 Lab request、第二 checkout 或 SHA 选择入口。Mac 不得启动本地 ADB、Gradle、Android Studio、模拟器或 scrcpy。
...
- 若改动触及 Android 权限、生命周期、Capacitor 插件、intent、安装 / 启动链路，或问题只会在模拟器 / 设备上暴露，必须由 controller 对 LAN Git 做普通 `dev` push，再由 Windows pull 并执行对应 `quality:android:device` / preview；Mac 本地不得替代该验收。
