...
DnsSdInput ReadInput(const Napi::CallbackInfo& info, const std::string& kind) {
  ...
  Napi::Value interface_index = object.Get("interfaceIndex");
  if (interface_index.IsNumber()) {
    input.interface_index = interface_index.As<Napi::Number>().Uint32Value();
  }
  if (kind == "browse") return input;
  input.name = StringField(object, "name");
  if (kind == "resolve") return input;
...
Napi::Object WrapOperation(Napi::Env env, std::shared_ptr<NativeOperation> operation) {
  ...
  auto cancel = Napi::Function::New(env, [operation](const Napi::CallbackInfo&) {
    operation->Stop();
  });
...
Napi::Value Begin(const Napi::CallbackInfo& info, const std::string& kind) {
  ...
  operation->Start();
  return WrapOperation(info.Env(), std::move(operation));
}
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
  exports.Set("browse", Napi::Function::New(env,
    [](const Napi::CallbackInfo& info) { return Begin(info, "browse"); }));
  exports.Set("register", Napi::Function::New(env,
    [](const Napi::CallbackInfo& info) { return Begin(info, "register"); }));
  exports.Set("resolve", Napi::Function::New(env,
    [](const Napi::CallbackInfo& info) { return Begin(info, "resolve"); }));
  return exports;
}
