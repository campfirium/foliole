...
function appendStyleAttribute(attributes: string, css: string) {
  const styleMatch = attributes.match(/\sstyle=(["'])(.*?)\1/u);
  if (!styleMatch?.[0] || styleMatch.index === undefined) {
    return `${attributes} style="${escapeHtmlAttribute(css)}"`;
  }
  const quote = styleMatch[1] ?? '"';
  const value = styleMatch[2] ?? '';
  const nextStyle = ` style=${quote}${escapeHtmlAttribute(`${value};${css}`)}${quote}`;
...
function injectStartupTokensIntoHtml(html: string, startupCss: string, themeSource: 'dark' | 'light') {
  ...
  return injectStartupDocumentState(removeStartupBootScript(withStartupCss), themeSource, startupCss);
}
...
export function injectDevRendererIntoHtml(
  ...
  const withStartupState = replaceDevRendererEntryWithAsyncLoader(
    injectStartupTokensIntoHtml(html, startupCss, themeSource).replace('src="/src/main.tsx"', `src="${entryUrl}"`),
    entryUrl
  );
  if (withStartupState.includes('<base href=')) {
    return withStartupState;
  }
  return withStartupState.replace('<head>', `<head>\n    ${createDevRendererHtmlBaseTag(normalizedDevUrl)}`);
}
