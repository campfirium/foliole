...
function snapshot() {
  ...
  if (!group) return { assignment, group: null, mode: sourceMode.mode };
  if (sourceMode.conflictReasons.length || sourceMode.mode === 'off' || !canPrepareReadwiseOnThisHost()) {
    throw new Error('readwise_local_connection_unavailable');
  }
  return { assignment, group, mode: sourceMode.mode };
}
...
async function requestRemoteStop(peer: DesktopSyncGroupPeer, request: ReadwiseStopRequest) {
  ...
  const response = await fetch(`${peer.endpoint_url}${READWISE_OWNER_STOP_PATH}`, {
