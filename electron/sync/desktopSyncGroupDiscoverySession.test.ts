...
it('publishes found, changed, and lost until explicitly stopped', async () => {
  ...
  const fetchDiscovery = vi.fn(async () => new Response(JSON.stringify({
    ...
    provider_device_id: 'device-a', provider_device_name: 'Mac', provider_platform: 'darwin',
    protocol: CURRENT_SYNC_PROTOCOL_DESCRIPTOR, timeline_id: 'timeline-1', topology_role: 'anchor'
  })));
  ...
  const service = { addresses: ['192.168.0.12'], fqdn: 'daily._foliole-sync._tcp.local', name: 'Daily', port: 38641,
    txt: { device_id: 'device-a', group_id: 'group-1', group_tag: 'tag-1',
      provider_platform: 'darwin', runtime_instance_id: 'remote',
...
it('ends a failed resolved route without probing advertised address fallbacks', async () => {
  ...
  const fetchDiscovery = vi.fn(async (url: string | URL | Request) => {
    ...
    return new Response(JSON.stringify({
      group_display_name: 'Daily Group', group_id: 'group-1', group_tag: 'tag-1',
      protocol: CURRENT_SYNC_PROTOCOL_DESCRIPTOR, provider_device_id: 'device-a',
