...
export class DesktopSyncGroupJoinPrepareProvider {
  readonly #groupInfo: SyncGroupJoinGroupInfo;
  readonly #requests = new Map<string, StoredJoinRequest>();

  constructor(groupInfo: SyncGroupJoinGroupInfo) {
  ...
  async accept(requestId: string, nowMs = Date.now()) {
    ...
    const encryptedGroupInfo = await encryptCompanionPairingSecret({
      clientPublicKey: stored.request.ephemeral_public_key,
      credentialSecret: JSON.stringify(this.#groupInfo)
    });
