...
it('keys entries by object type and id together', () => {
  const result = diffSyncIndex(
    [
      createEntry({ object_id: 'shared-id', object_type: 'node', content_hash: 'hash-node' }),
      createEntry({ object_id: 'shared-id', object_type: 'node_reading', content_hash: 'hash-reading' })
    ],
    [
      createEntry({ object_id: 'shared-id', object_type: 'node', content_hash: 'hash-node' }),
      createEntry({ object_id: 'shared-id', object_type: 'node_reading', content_hash: 'hash-reading-remote' })
    ]
  );

  expect(result.inSync.map((entry) => entry.object_type)).toEqual(['node']);
  expect(result.inspectCandidates).toEqual([
    expect.objectContaining({
      local: expect.objectContaining({ object_type: 'node_reading', object_id: 'shared-id' })
    })
  ]);
});
