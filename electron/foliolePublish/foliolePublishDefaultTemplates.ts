...
<body data-foliole-publish-site data-page-kind="{{ page.view }}">
  <main class="shell" id="main">
    {% if page.view == "home" %}
    <section class="view home-view{% if page.topics.size == 0 %} is-empty{% endif %}">
      <header class="page-header"><h1 class="home-title">{{ site.title }}</h1>{% if page.topics.size > 0 %}${SITE_NAV}{% else %}${EMPTY_HOME_NAV}{% endif %}</header>
      {% if page.topics.size > 0 %}
      <div class="topic-stream">
        {% for topic in page.topics %}
        <article class="topic-entry">
          <h2 class="topic-title"><a href="{{ page.depth }}{{ topic.path }}">{{ topic.title }}</a></h2>
          {% if topic.preview != "" %}<div class="topic-segment">{{ topic.preview | raw }}{% if topic.has_more %}<a class="continuation" href="{{ page.depth }}{{ topic.path }}" aria-label="Read {{ topic.title }}">…</a>{% endif %}</div>{% endif %}
        </article>
        {% endfor %}
      </div>
      {% else %}
    ...
    <article class="view article-view">
      ...
      <footer class="article-footer">
        <div class="meta" aria-label="Topic information">
          ...
          {% if page.id %}{% for topic in site.topics %}{% if topic.id == page.id %}
          {% if topic.categories.size > 0 %}<div class="meta-row"><span class="meta-key">Category</span><span class="meta-values">{% for category in topic.categories %}<a href="{{ page.depth }}categories/{{ category.slug }}/">{{ category.name }}</a>{% endfor %}</span></div>{% endif %}
          {% if topic.tags.size > 0 %}<div class="meta-row"><span class="meta-key">Tags</span><span class="meta-values">{% for tag in topic.tags %}<a href="{{ page.depth }}tags/{{ tag.slug }}/">#{{ tag.name }}</a>{% endfor %}</span></div>{% endif %}
          {% endif %}{% endfor %}{% endif %}
        </div>
        ${SITE_NAV}
      </footer>
    ...
    <section class="view index-view">
      ...
      <div class="year-groups">
        {% for group in page.groups %}<section class="year-group"><h2 class="year-title">{{ group.label }}</h2><ol class="index-list">
          {% for topic in group.topics %}<li class="index-row"><time class="index-date" datetime="{{ topic.updated_at }}">{{ topic.updated_at | date: "%m-%d" }}</time><a class="index-title" href="{{ page.depth }}{{ topic.path }}">{{ topic.title }}</a></li>{% endfor %}
        </ol></section>{% endfor %}
      </div>
      {% elsif page.view == "categories" %}
      <ul class="category-list">{% for term in page.terms %}<li><a class="category-link" href="{{ term.slug }}/">{{ term.name }}</a></li>{% endfor %}</ul>
      {% elsif page.view == "tags" %}
      <div class="tag-cloud">{% for term in page.terms %}<a class="tag-link" data-count="{{ term.count }}" href="{{ term.slug }}/">#{{ term.name }}</a>{% endfor %}</div>
