...
- 凡本轮改动会进入 Windows 桌面运行时或改变桌面用户可见行为，必须先完成本轮相关前置验证并运行 L0 `npm run quality:fast:native`，再完成一次 L1 验收。首选 L1a Hidden Native：新增或复用本轮任务相关 spec，并运行 `npm run test:e2e:desktop:native:hidden -- <spec>`；若行为不适合 hidden-capable 自动化，再升级到 L1b `npm run test:e2e:desktop:native:visible -- <spec>` 或 L2 人工验收。只改文档、agent 规则、只读诊断、测试代码或脚本内部逻辑，且不改变桌面运行时行为时，可跳过 L0 / L1，但最终汇报必须写明跳过原因。
...
- Agent 日常 L1 自动化验收先按验收目标选入口：纯 renderer / Web UI 行为若属于桌面产品表面，仍优先使用 `npm run test:e2e:desktop:native:hidden -- <spec>` 覆盖真实 Electron renderer；只有非桌面表面才使用 headless browser。Windows 原生 Codex 会话必须运行本轮任务相关的 hidden-capable Playwright spec，不得用无参 hidden health 替代功能验收。
...
- L1a Hidden Native 验收只覆盖 renderer / preload bridge / IPC / 临时 sqlite / navigation / layout 等不依赖真实桌面 focus 的当前任务行为；只要本轮桌面产品行为可被 hidden-capable spec 断言，就必须新增或复用 targeted spec 并运行。若本轮没有 hidden-capable 桌面行为，最终汇报说明跳过 L1a 的原因；若行为依赖 focus、窗口拖拽、系统 dialog、tray、notification、installer / updater、真实菜单栏，则必须升级到 L1b 可见窗口自动验收、L2 人工验收或发布专项验收。
