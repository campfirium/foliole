...
- A5 设备自动化必须由固定 device adapter 消费 Windows 单仓 pull 后的 `dev` 和目标 A5 identity；adapter 尚未可用时停止设备动作，不恢复旧 Lab request。清数据、re-pair、Lab 根外读取、提权、防火墙或系统级修改必须在执行前返回 `approval_required`，不得提供远程 approval bypass。
...
- Windows 使用一个普通 `dev` Git 仓库作为 Windows 桌面与 A5 Android 的开发现场；每次动作前只做 `git pull --ff-only lan dev`，Windows desktop dev 负责 native client runtime，A5 dev-server / APK 安装由 Android device adapter 负责。
