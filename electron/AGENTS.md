...
- T1 隐藏检查只覆盖 renderer / preload bridge / IPC / 临时 sqlite / navigation / layout 等不依赖真实桌面 focus 的当前任务行为；只要本轮桌面产品行为可被 hidden-capable spec 断言，就必须新增或复用 targeted spec 并运行。若本轮没有 hidden-capable 桌面行为，最终汇报说明跳过 T1 的原因；若行为依赖 focus、窗口拖拽、系统 dialog、tray、notification、installer / updater、真实菜单栏，则必须升级到 T2 可见检查、T3 人工检查或发布专项验收。
