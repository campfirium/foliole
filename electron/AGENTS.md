...
- Windows 开发机使用普通局域网 SSH 进入 PowerShell；Mac 通过 `node scripts/windows/windows-dev-control.mjs --host <host> <appearance|build|capture-annotation|deploy|live|pair-sync-recover|secondary|verify>` 对 LAN Git 做普通 `dev` push，Windows 单仓再执行 `git pull --ff-only lan dev` 与固定前台动作。入口不解析或比对 SHA。
...
- Windows 使用一个普通 `dev` Git 仓库作为 Windows 桌面与 A5 Android 的开发现场；每次动作前只做 `git pull --ff-only lan dev`，Windows desktop dev 负责 native client runtime，A5 actions 由固定 Android device adapter 负责。
