...
- Windows 原生 Codex 检查或控制现有 Windows Electron 客户端时，优先使用 `npm run windows:client:native -- <status|start|stop|restart|full-restart>`；该入口直接调用既有 `restart-electron-dev.ps1`，避免 Bash `wslpath`、WSL mirror 默认目录和 PowerShell inline command 转义。
...
- `npm run windows:preview` 只在根 `AGENTS.md` 的连续推进 / 阶段验收规则允许，且 `.lab/internal/runtime/windows-preview.flag` 为 `ON`、用户当次明确要求 Windows 预览、或用户当次要求阶段验收时执行。
