import type { ImportManagerSettings, ImportManagerSourceDraft, ReadwiseSourceKind } from '../../lib/core/import/importManagerSettings.js';
import type {
...
import { resolveReadwiseSourceImportDecision, resolveReadwiseSourceSignature } from './readwisePreparedImport.js';
...
function isReadwiseImportSource(source: ImportManagerSourceDraft): source is ReadwiseImportSource {
  return Boolean(source.kind) && source.primaryPath.trim().length > 0 && source.highlightPath.trim().length > 0;
}

function resolveTrackingStatus(ruleId: string, source: DirectoryImportSourceDescriptor, highlightChanged: boolean, primaryChanged: boolean) {
  const existingItem = readKeepImportItem(ruleId, source.sourceName);
  if (!existingItem) {
    return 'new' as const;
  }
  const nodeState = existingItem.last_node_id ? readKeepImportNodeState(existingItem.last_node_id) : null;
...
async function previewReadwiseSource(readwiseSource: ReadwiseImportSource, settings: ImportManagerSettings) {
  try {
    const sources = await discoverDirectoryImportSources(readwiseSource.primaryPath, { supportedKinds: ['markdown'] });
...
export async function previewReadwiseReaderImport(settings = loadImportManagerSettings()): Promise<ReadwiseSyncPreviewResult> {
  const entries = (
    await Promise.all(settings.readwiseSources.filter(isReadwiseImportSource).map((source) => previewReadwiseSource(source, settings)))
  ).flat();
  return {
    entries,
    ...
    with_highlights_count: entries.filter((entry) => entry.highlight_type === 'with_highlights').length,
    without_highlights_count: entries.filter((entry) => entry.highlight_type === 'without_highlights').length,
    write_count: entries.filter((entry) => entry.destination !== 'off' && entry.status !== 'failed').length
  };
}
