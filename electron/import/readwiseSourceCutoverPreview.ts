...
import { loadReadwiseSourceCutover, loadReadwiseSourceMigrationProgress } from '../database/readwiseSourceCutover.js';
...
export async function previewReadwiseSourceCutover(): Promise<NativeReadwiseSourceCutoverPreview> {
  ...
  if (current && (current.status === 'migration-in-progress' || completed)) {
    ...
    const download = readCutoverDownloadProgress(connectionRef);
    const update = loadReadwiseSourceMigrationProgress();
    return {
      completed_count: indexing ? download.completed : update.completedCount,
      ...
      total_count: indexing ? download.total : update.totalCount
    };
  }
