...
it('keeps opaque references and historical Host snapshots stable while rekeying membership identity', () => {
  ...
  expect(sqlite.prepare('SELECT version_id, parent_version_id, host_name FROM node_sync_versions ORDER BY version_id').all())
    .toEqual([
      { host_name: 'Maci', parent_version_id: null, version_id: 'ver_0' },
      { host_name: 'Maci', parent_version_id: 'ver_0', version_id: 'ver_1' }
    ]);
  ...
  expect(sqlite.prepare('SELECT current_version_id, last_modified_by_host_name FROM nodes WHERE id = ?').get('topic-b'))
    .toEqual({ current_version_id: 'ver_1', last_modified_by_host_name: 'Maci' });
  expect(sqlite.prepare('SELECT current_version_id, last_modified_by_host_name FROM sync_object_state').get())
    .toEqual({ current_version_id: 'ver_1', last_modified_by_host_name: 'Maci' });
  expect(sqlite.prepare('SELECT base_version_id, result_version_id, host_name FROM sync_change_log').get())
    .toEqual({ base_version_id: 'ver_0', result_version_id: 'ver_1', host_name: 'Maci' });
...
it('rejects an invalid identity without changing opaque history', () => {
  ...
  expect(sqlite.prepare("SELECT COUNT(*) FROM node_sync_versions WHERE host_name = 'Maci'").pluck().get()).toBe(2);
...
function seedLocalHistory() {
  sqlite.exec(`
    ...
    INSERT INTO nodes (
      id, parent_id, kind, title, content, current_version_id, last_modified_by_host_name,
