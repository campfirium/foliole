...
it('transfers the unique desktop Host scope while preserving permanent state', () => {
  ...
  sqlite.exec(`
    ...
    INSERT INTO setting_records VALUES
      ...
      ('readwise_import_settings','host','windows','desktop','Old Mac','{"version":5,"readwiseRootPath":"/Readwise","readwiseSourceMode":"folder","autoImportPolicy":"manual","autoImportPolicyVersion":3}','readwise-hash','old',NULL);
  ...
  expect(sqlite.prepare('SELECT key, host_name, value_json FROM setting_records ORDER BY key').all())
    .toEqual([
      { host_name: 'New Mac', key: 'readwise_import_settings', value_json: '{"version":5,"readwiseRootPath":"/Readwise","readwiseSourceMode":"folder","autoImportPolicy":"manual","autoImportPolicyVersion":3}' },
