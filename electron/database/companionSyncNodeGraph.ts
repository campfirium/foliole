...
export interface StoredVersionRow extends DbRow {
...
export async function loadCurrentSyncNodeRecord(
  ...
  const [row] = await port.query<StoredVersionRow & { current_version_id: string }>(
    `SELECT v.*, n.current_version_id FROM nodes n
...
export async function loadMergeBase(port: DbPort, leftId: string, rightId: string) {
  ...
  const [row] = await port.query<StoredVersionRow>(
...
async function storedVersionToRecord(port: DbPort, row: StoredVersionRow): Promise<NativeSyncNodeRecord> {
