...
const MANAGED_DESTINATION = String.raw`(?:foliole-(?:auto|manual|rollback)-\d{6}-\d{6}(?:-\d+)?|foliole-auto-backup-\d{6}-\d{6}|manual-${TIMESTAMP}|pre-(?:compact|migration|restore)-${TIMESTAMP})\.db\.gz`;
