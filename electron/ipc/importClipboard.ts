...
async function runClipboardImageImport(args?: NativeTextImportArgs) {
  ...
  const preparedImage = prepareCanonicalImageAttachment(bytes);
  if (!preparedImage) throw new Error('The clipboard image bytes are not supported.');
  const { hash, mimeType, storageKey } = preparedImage;
