...
it('falls back to compiled preload beside electron-dist root', () => {
  const runtimeDir = path.join('C:', 'dev', 'foliole', 'electron-dist', 'electron');
  const compiledPreload = path.join('C:', 'dev', 'foliole', 'electron-dist', 'preload.cjs');

  expect(resolvePreloadScriptPath(runtimeDir, createExistsSync([compiledPreload]))).toBe(compiledPreload);
});
