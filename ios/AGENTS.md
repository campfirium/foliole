...
- iOS 公开最小流程：`npm run android:web:build`、`npx cap sync ios`、`node -e "import('./scripts/ios/ios-local-storage.mjs').then(({ prepareIosAcceptanceCache }) => prepareIosAcceptanceCache(process.cwd()))"`、`xcodebuild -project ios/App/App.xcodeproj -scheme App -configuration Debug -destination 'generic/platform=iOS Simulator' -derivedDataPath .cache/ios-acceptance-build/DerivedData CODE_SIGNING_ALLOWED=NO build`。通用编译不得把 DerivedData 写入 `.tmp/artifacts/<task-entry>`。
...
- Fri 开发构建与真机操作的公开入口是 `node scripts/ios/fri-dev-workflow.mjs`。它只消费当前 cwd，先由 `scripts/ios/fri-app-retention.mjs` 精确保留正式 App 与固定开发 / 验收身份并清除旧调试身份，再依次执行 companion build、`npx cap sync ios`、固定 wired Fri 的签名 XCUITest，并把独立 `com.foliole.ios.devworkflow` 开发应用重新留在前台；T152 真机入口固定复用 `com.foliole.ios.t152acceptance`，只在每个隔离 cell 开始前重置该验收容器。最小验证先跑对应窄测试与 lint、通用无签名 iOS build，再在 wired Fri 上检查 `.xcresult` summary、设备详情与导出截图。不得用该入口触及正式 `com.foliole.ios`、改用无线 / Simulator，或插入 Git 源码准备。
