...
- 当前正式原生能力包含 `FolioleCompanionBootstrap` 与既有格式同步包的本地 apply；bootstrap 必须通过 Capacitor SQLite 初始化共享 schema、持久化稳定的 iOS 设备身份并仅在数据库可读写后报告 ready，sync-pack apply 必须复用共享 `DbPort` 语义并将游标持久化到 `companion_meta`。下载、配对及其他 Android-only runtime 能力不得隐式回退为 Web 实现。
