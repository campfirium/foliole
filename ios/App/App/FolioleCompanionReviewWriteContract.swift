...
struct FolioleCompanionReviewWriteContract {
    ...
    let reviewLogColumnKeys: [String]
    let reviewLogInsertSQL: String
    let reviewUpsertSQL: String
    ...
    init(bundle: Bundle = .main) throws {
        ...
        let review = try FolioleCompanionJsonContract.object(
            ["queries", "syncPayloadNodeReview", "syncPayload"], in: query
        )
        let reviewLog = try FolioleCompanionJsonContract.object(["queries", "syncReviewLog"], in: query)
        ...
        let logMutations = try FolioleCompanionJsonContract.object(["syncApplyMutations", "reviewLog"], in: mutation)
        ...
        defaults = Self.values(review, [
            "difficulty": "defaultDifficulty", "elapsedDays": "defaultElapsedDays",
            "lapses": "defaultLapses", "reps": "defaultReps",
            "scheduledDays": "defaultScheduledDays", "stability": "defaultStability",
            "state": "defaultState"
        ])
        payloadKeys = try Self.strings(review, [
            "cardAfter": "reviewLogCardAfterInputKey", "cardBefore": "reviewLogCardBeforeInputKey",
            "difficulty": "difficultyPayloadKey", "due": "duePayloadKey",
            "elapsedDays": "elapsedDaysPayloadKey", "grade": "reviewLogGradeInputKey",
            "input": "inputPayloadKey", "lapses": "lapsesPayloadKey",
            "lastReviewAt": "lastReviewAtPayloadKey", "nodeId": "nodeIdPayloadKey",
            "reps": "repsPayloadKey", "reviewLog": "reviewLogInputPayloadKey",
            "reviewedAt": "reviewLogReviewedAtInputKey", "schedulerVersion": "reviewLogSchedulerVersionInputKey",
            "scheduledDays": "scheduledDaysPayloadKey", "stability": "stabilityPayloadKey",
            "state": "statePayloadKey"
        ])
        objectType = try FolioleCompanionJsonContract.string(["syncObjectTypes", "nodeReview"], in: sync)
        resultKeys = try Self.stringMap(["syncWrite", "resultKeys"], sync)
        reviewLogColumnKeys = try Self.columnKeys(reviewLog)
        reviewUpsertSQL = try Self.statement("reviewUpsertMutationName", learning, statements)
        reviewLogInsertSQL = try Self.statement("insertMutationName", logMutations, statements)
    ...
    private static func columnKeys(_ root: [String: Any]) throws -> [String] {
        guard let columns = root["columns"] as? [[String: Any]] else {
            throw FolioleCompanionJsonContract.error("syncReviewLog.columns")
        }
        return try columns.map { try string("key", $0) }
    }
}

extension FolioleCompanionReviewWriteContract: FolioleCompanionLearningSyncStateContract {}
