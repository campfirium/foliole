...
enum FolioleCompanionZlib {
    static func deflate(_ plain: Data) throws -> Data {
        var stream = z_stream()
        let initialized = deflateInit_(&stream, Z_DEFAULT_COMPRESSION, ZLIB_VERSION, Int32(MemoryLayout<z_stream>.size))
        guard initialized == Z_OK else { throw error("deflate init", initialized) }
        defer { deflateEnd(&stream) }
        return try plain.withUnsafeBytes { source in
            ...
            return try drainDeflate(&stream)
        }
    }
    ...
    private static func drainDeflate(_ stream: inout z_stream) throws -> Data {
        let capacity = 256 * 1024
        ...
        repeat {
            ...
            if written > 0 { output.append(contentsOf: buffer.prefix(written)) }
            guard status == Z_OK || status == Z_STREAM_END else { throw error("deflate", status) }
        } while status != Z_STREAM_END
        return output
    }
