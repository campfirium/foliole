...
test('folder custom order is edited in the topic column and only displayed in content cards', async ({
  ...
  await column.getByRole('button', { name: /^(Sort list by Date modified|按修改日期排序列表)$/ }).click();
  await desktopWindow.getByRole('menuitem', { name: /^(Custom order|自定义顺序)$/ }).click();
  await expect(column.getByRole('button', {
    name: /^(Sort list by Custom order|按自定义顺序排序列表)$/
  })).toBeVisible();
  ...
  if (!sourceBox || !targetBox) throw new Error('missing custom-order topic row bounds');
  await desktopWindow.mouse.move(sourceBox.x + sourceBox.width / 2, sourceBox.y + sourceBox.height / 2);
  await desktopWindow.mouse.down();
  await desktopWindow.mouse.move(sourceBox.x + sourceBox.width / 2, sourceBox.y - 8, { steps: 4 });
  await desktopWindow.mouse.move(targetBox.x + targetBox.width / 2, targetBox.y + 1, { steps: 8 });
  ...
  await folderView.getByRole('button', {
    name: /^(Sort list by .+|按.+排序列表)$/
  }).click();
  await desktopWindow.getByRole('menuitem', { name: /^(Custom order|自定义顺序)$/ }).click();
  await expect(folderView.getByRole('button', {
    name: /^(Sort list by Custom order|按自定义顺序排序列表)$/
  })).toBeVisible();

  const cards = folderView.getByRole('list', { name: /^(Folder contents|文件夹内容)$/ });
  await expect(cards.getByRole('button', { name: 'Open Beta topic' })).toBeVisible();
  await expect(cards.getByRole('button', { name: 'Open Alpha topic' })).toBeVisible();
  await expect(cards.getByRole('button')).toHaveCount(2);
  await expect(cards.getByRole('button').nth(0)).toHaveAttribute('aria-label', 'Open Beta topic');
  await expect(cards.getByRole('button').nth(1)).toHaveAttribute('aria-label', 'Open Alpha topic');
  await expect(cards.getByRole('button').nth(0)).not.toHaveAttribute('draggable');
  await expect(cards.getByRole('button').nth(1)).not.toHaveAttribute('draggable');
