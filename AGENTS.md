...
- 非连续推进且非阶段验收时，预览 flag 为 `OFF` 则不执行对应预览，除非用户当次明确要求；预览 flag 为 `ON` 且本轮改动存在用户可见预览验收面、相关验证通过后，默认执行与受影响宿主匹配的预览。仅改 agent 规则、文档、测试、脚本、内部 spec 或无客户端可见面的代码时，不因 flag 为 `ON` 自动预览，除非用户当次明确要求。
