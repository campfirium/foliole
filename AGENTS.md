...
5. 只改当前任务相关文件；发现结构性阻塞时，先写回 TODO，再决定是否提升优先级。
6. `.lab/agent/TODO.md` 是未完成工作的唯一真实来源；`.lab/agent/DONE.md` 只记录已完成项。
...
- `AGENTS.md`、`.lab/specs/**`、`.lab/agent/workflow.md` 可提交；TODO / DONE / flags / handoff / iteration log 等运行态文件默认不提交。
