...
- 若规则与用户当次最新指令冲突，以用户最新指令为准。当前代码是现状行为、迁移输入与可运行边界的事实来源；执行审定方案时，`prepare` 保持当前生产真相，`cutover` 以该轴的审定目标语义切换全部消费方，`n/a` 以当前闭环结果与禁止范围裁决本轮改变。代码、测试或 controller 与 cutover 目标冲突时不得借旧实现改写方案；只有用户要求、平台硬边界、数据安全或同一现场证据证伪正式合同时才停工返 `/2`。
...
- 关联实施方案的任务标题从方案读取：`T<n>-0 <方案短标题>`、`T<n>-<k> <闭环结果标题>`、`T<n>-0 验收 <方案短标题>`。
- 实施方案工作流只认 `/2` 与 `/3`：`/2` 一次写完整方案和顺序闭环，reviewer 要求用户裁决或改变模型时必须停；`/3` 每个任务只执行一个闭环，全部完成后才进入 `/4`。具体格式与执行规则只写在对应 skill，不在 AGENTS 重复。
...
- Windows 开发以当前 Mac 工作区的 `dev` 为唯一源码真相：Mac controller 将它精确覆盖到只作传输镜像的 LAN Git `dev`，再精确覆盖固定 Windows `dev` 仓库；Windows 源码漂移不构成独立工作现场，由固定 controller 丢弃后跟随权威镜像。LAN Git 与 Windows 不向 Mac 提供或合并源码，也不决定候选；Windows trusted runtime revision 只证明已同步 Mac `dev` 的产品 runtime 等价性，最终验收源码 revision 仍取 controller 已同步到固定 Windows 仓库的当前 Mac 完整提交，macOS 从该提交的 Git archive 对齐。Git 失败直接报告。
- Windows 固定仓库服务 Windows 桌面、Windows 专属 A5 联动与最终跨宿主验收；Android 日常开发、固定 A5 调试和 Mac desktop DEV library 同步留在 Mac 当前工作区。Windows 不提交或推送源码上游，不建立 candidate、scratch 或第二份源码现场，也不 stash、合并或修复 Windows 源码；ignored runtime、library 与工具缓存不属于源码覆盖范围。
...
8. 执行中需要改变已审定的 schema / bridge / IPC / 协议 / 依赖 / 后台任务 / fallback / migration / 双写 / scan / poll 或当前验收句时，必须停止，取得用户裁决并由 `/2` 修订方案；闭环内部实现调整不升级。
9. 方案执行中发现的新问题先按当前方案的合同责任归属：同一合同轴留在当前闭环；同一最终结果与综合验收所需的新轴交 `/2` 修订原方案；只有原方案无需该结果仍可验收的独立结果才交 `task-seed`。不得按文件、失败、平台或 controller 边界转移 owner，也不存在受另一个 Track 治理的从属 Track；无法唯一归属时停给 `/2`。
10. 只有用户明确要求 spike，或任务评估把某段代码标为 spike / diagnostic / fallback / one-off migration 时，才允许写临时验证代码；否则临时代码不得接入正式入口。
11. 日常开发与提交前宿主试跑可按局部规则消费工作区；闭环最终宿主验收只消费已完成前置验证、已提交且冻结的完整 revision，多宿主必须绑定同一 accepted tip。真实设备 / 数据旅程开始后，不得在同一次验收尝试中修改产品代码、验收控制器或成功判据。红灯若需要源码改动，当前尝试立即失效：先保留证据并退出旅程，回到实现与前置验证，重新建立验收基线后从头执行；不得拼接改动前后的局部证据宣称闭环完成。
12. 难恢复副作用失败后，只有新证据改变下一安全动作且基线重建后才可重试；否则留在当前 `/3`。正式合同或授权失效时停给用户并由 `/2` 修订；数据损坏、意外删除或静默覆盖先保护现场。
...
最终汇报默认使用 `C / V / R / pushed`：需要用户介入时，首句直接写用户下一步要做的事，原因和证据后置；`C` 写用户可用结果与已确认根因；`V` 只用自然语言写验证结论、必要证据或必要的跳过原因，合并同类成功项，默认不展开命令清单、workflow 阶段、内部判断、已消解阻塞或证据台账，仅在验证失败、用户追问或审计要求时展开；`R` 只写真正剩余风险，没有风险时省略。不列内部字段、数据库对象或可选后续。
