...
- 最终汇报面向用户验收，不面向代码审计；默认说明“完成了什么”，不逐条罗列文件路径、函数名、测试命令或实现细节，除非用户追问或存在失败、风险、未完成项必须说明。
- 当需要用户验收时，最终汇报必须写明“你可以这样验收”，包含具体查看位置、操作路径和通过标准。
- 有未验证、失败、数据 / 安全影响、技术债或与本轮目标直接相关的范围边界时，必须写“风险和限制”；禁止把无关未做事项塞进汇报。
- 只有需要用户选方向、确认取舍或提供信息时，才写“需要你决定”。
- 不默认写“下一步”；只有存在未完成项、阻塞、实施说明的明确后续步骤或用户决策点时，才说明后续动作，禁止把可选建议伪装成任务主线。
- `windows:preview` / `android:preview` 必须报告实际命令和最终 `status` 字段。
