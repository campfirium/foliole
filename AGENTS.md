...
- 验证方式按任务模式区分：
  - 对话协作模式：只要本轮是用户与 AI 的交互式改动任务，并且本轮实际修改了任何仓库文件，则在相关验证通过后、向用户汇报前，必须追加执行一次预览；不得把预览推迟到“对话结束时统一执行”，也不得只在多轮中的最后一轮才跑。默认入口为 `npm run verify:preview`，若只需单独刷新 Windows 客户端，也可执行 `npm run windows:preview`。
