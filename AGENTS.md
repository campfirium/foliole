...
- 预览选择规则如下：
- 影响 Electron / Windows / preload / IPC / sqlite / desktop runtime 且 Windows 预览开启：执行 `npm run windows:preview`
...
- `C` 写用户可理解的结果，并用半句说明本轮为什么要改、根因或触发点；禁止只复述“完成了什么”。不写函数名、字段名、路径、命令或实现细节；句式按“某个用户问题现在会怎样 + 因为什么原因/场景”写，例如“安卓同步前会先留一份手机本地库备份，因为同步覆盖前缺少可回退点”。若根因未确认，只写已确认现象和当前处理边界，不把猜测写成事实。
