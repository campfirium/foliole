...
10. 日常开发与提交前宿主试跑可按局部规则消费工作区；闭环最终宿主验收只消费已完成前置验证、已提交且冻结的完整 revision，多宿主必须绑定同一 accepted tip。真实设备 / 数据旅程开始后，不得在同一次验收尝试中修改产品代码、验收控制器或成功判据。红灯若需要源码改动，当前尝试立即失效：先保留证据并退出旅程，回到实现与前置验证，重新建立验收基线后从头执行；不得拼接改动前后的局部证据宣称闭环完成。
11. 难恢复副作用失败后，只有新证据改变下一安全动作且基线重建后才可重试；否则留在当前 `/3`。正式合同或授权失效时停给用户并由 `/2` 修订；数据损坏、意外删除或静默覆盖先保护现场。
