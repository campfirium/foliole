...
1. 任何影响用户可见行为的代码改动完成后，Agent 必须执行 `npm run windows:dev:native`，确保变更进入 Windows 客户端运行态，不得仅停留在本地构建或测试通过。
2. 若改动涉及窗口壳层/标题栏/启动流程/Tauri 配置/原生权限，必须执行 `npm run windows:dev:restart`，不得仅执行 `sync`。
3. 对用户宣称“已可在 Windows 客户端验证”前，必须回报本次执行的实际命令与最终状态字段（如 `status: SYNCED` 或 `status: RESTARTED`）。
4. 若同步或重启失败，必须先修复同步链路再继续功能结论输出；禁止以“代码已改完”替代客户端可见结果。
...
6. 仅在用户确认后进入实施与验证；实施后按 SOP 更新 `.lab/agent/iteration-log.md`。
