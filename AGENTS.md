...
- 关联已编号正式实施方案的 Codex 任务标题必须同时保留编号和方案内的可读名称：方案级任务使用 `T<n>-0 <方案短标题>`，执行任务使用 `T<n>-<k> <闭环结果标题>`，最终验收使用 `T<n>-0 验收 <方案短标题>`；编号与名称都从方案读取，禁止只写编号。
- 一份正式实施方案一次性保存最终结果、系统模型与不变量、按依赖排序的多个闭环、综合验收和已完成事实。最早未完成且前置满足的闭环是当前唯一闭环；每个闭环只写单一结果、允许范围、禁止范围和验收，不维护 rolling 状态机或系统覆盖表。
- `/3` 只执行 `Plan review: approved` 的方案。同一验收句未成立时，不因缺口、提交、文件、平台、重试或耗时数量换任务；失败、诊断、重试、提交和被放弃方法留在持有任务或证据载体，不进入方案。正式模型、边界、验收或授权失效时停给用户并由 `/2` 修订同一方案，审定后仍由原任务继续。
- reviewer 返回 `needs-user-decision`，或提出实体、身份、所有权、数据流、不变量及用户授权变化时，方案必须保持 blocked 并停给用户；规划任务不得自行吸收后放行或创建 `/3`。所有普通闭环完成后才进入 `/4`。
...
8. 执行中发现不再属于当前验收句，或需要改变已审定的 schema / bridge / IPC / 协议 / 依赖 / 后台任务 / fallback / migration / 双写 / scan / poll 路线，必须停止实现并进入 `/2` 方案修订、`NEEDS_EVAL` 或 `STOP_CONFIRM`；先取得用户裁决并更新、会审正式合同再继续。已在方案中审定、且不改变正式边界的闭环内部实现不重复升级。
...
11. 真实安装、配对、同步、恢复或其他难恢复副作用失败后，只有新证据已经改变下一安全动作且验收基线重新成立时才可重试；同一路线无新事实不得重复等待。缺口数量不触发换任务；仍属同一验收句且正式路线有效时留在当前 `/3`。正式 contract / 路线 / 授权失效时退出旅程，停给用户并在当前任务执行 `/2` 方案修订；所有授权路线已否证或剩余路线都需新授权时停止。数据损坏、意外删除或静默覆盖必须先保护现场和恢复边界。
