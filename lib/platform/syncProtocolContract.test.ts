...
describe('syncProtocolContract', () => {
  it('accepts the exact v5 descriptor and returns a negotiated version', () => {
  ...
  it.each([
    ...
    [descriptor({ min_supported_version: 6 }), 'protocol_metadata_invalid'],
    [descriptor({ max_supported_version: 6, min_supported_version: 6, version: 6 }), 'protocol_version_unsupported']
  ...
  it('reports missing required capabilities', () => {
    expect(evaluateSyncProtocolCompatibility(descriptor({ capabilities: [] }))).toEqual({
      missing_capabilities: [
        'identity-bearing-attachment-tombstone-v1', 'author-host-snapshots-v1', 'device-delivery-receipts-v1',
  ...
  it('keeps mDNS TXT as a bounded version hint and leaves capabilities to discovery', () => {
    ...
    expect(hint).toEqual({ max_supported_version: 5, min_supported_version: 5, version: 5 });
...
it('requires the display-name contract as part of the exact v5 generation', () => {
  ...
  expect(evaluateSyncProtocolCompatibility(CURRENT_SYNC_PROTOCOL_DESCRIPTOR))
    .toMatchObject({ negotiated_version: 5, status: 'compatible' });
});
