import {
  ...
  resolveBestOriginalTextCandidate
} from './textAnchorTextSearch.js';

export { repairTextAnchorLocatorInContent };
...
function clampTextAnchorSelection(_content: string, locator: TextAnchorLocator) {
...
function resolveMaximumRemappedSpan(args: {
  ...
  const changeIsInsideAnchor = args.previousChangeFrom >= args.locator.from && args.previousChangeTo <= args.locator.to;
  return changeIsInsideAnchor ? Math.max(0, previousSpan + nextChangeLength - previousChangeLength) : previousSpan;
}
...
function resolveOriginalTextInChangedRange(args: {
  ...
  return resolveBestOriginalTextCandidate({
...
function remapTextAnchorLocatorThroughContentChange(
  ...
  if (nextContent.slice(locator.from, locator.to) === locator.originalText) {
    return locator;
  }
  ...
  return {
    from: normalizedFrom,
    originalText: normalizedFrom === normalizedTo ? locator.originalText : nextContent.slice(normalizedFrom, normalizedTo),
    to: normalizedTo
  };
}
