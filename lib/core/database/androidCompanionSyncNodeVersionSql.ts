export const ANDROID_COMPANION_SYNC_NODE_VERSIONS_SQL =
  ...
  "WHERE v.device_id = ?1 AND (?3 = '' OR ?4 = '' OR v.created_at > ?5 OR (v.created_at = ?6 AND v.version_id > ?7)) " +
  "AND NOT EXISTS (SELECT 1 FROM sync_delivery_receipts receipt WHERE receipt.peer_id = ?2 " +
  ...
  "WHERE t.device_id = ?1 AND (?3 = '' OR ?4 = '' OR t.created_at > ?5 OR (t.created_at = ?6 AND t.version_id > ?7)) " +
  "AND NOT EXISTS (SELECT 1 FROM sync_delivery_receipts receipt WHERE receipt.peer_id = ?2 " +
