  ...
  return driver.prepare(
    `INSERT INTO nodes (
     id, parent_id, kind, priority, desired_retention, enable_short_term, sequential_reading_enabled, shelved_at, manual_child_order, title, is_title_manual, hide_title_heading,
       content, body_blob_hash, opening_text, virtual_filter, reveal, anchor_link, image_regions, image_sources, position,
     ...
     ON CONFLICT(id) DO UPDATE SET
       ...
       image_sources = json_patch(COALESCE(nodes.image_sources, '{}'), COALESCE(excluded.image_sources, '{}')),
