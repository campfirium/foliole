...
export function deriveNodeTitleForCloze(promptContent: string, answerContent: string) {
  ...
  if (promptTitle !== UNTITLED_NODE_TITLE) return promptTitle;
  const answerTitle = deriveNodeTitleFromContent(answer);
  return answerTitle !== UNTITLED_NODE_TITLE ? answerTitle : deriveNodeTitleFromContent(prompt);
}
...
export function deriveNextUntitledNodeTitle(titles: string[]) {
  ...
  for (const title of titles) {
    ...
    if (sequence !== null) nextSequence = Math.max(nextSequence, sequence + 1);
  }
...
function stripMarkdownPrefixes(value: string) {
  return value.trim().replace(/^>\s*/, '').replace(/^[-*+]\s+/, '').replace(/^\d+\.\s+/, '').replace(/^#{1,6}\s+/, '');
}
