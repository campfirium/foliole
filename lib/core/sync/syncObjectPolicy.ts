...
export const SYNC_OBJECT_POLICIES: readonly SyncObjectPolicy[] = [
  ...
  policy('attachment', 'attachment', 'resource', 'workspace', 'lww', ['attachments'], true),
  ...
  policy('node_reading.reading_position', 'node_reading', 'reading', 'host', 'host_private', ['node_reading_host_state'], true, 'diagnostic'),
  policy('node_reading_host_state', null, 'reading', 'host', 'host_private', ['node_reading_host_state'], true, 'diagnostic'),
  ...
  policy('view_state.node', 'view_state', 'ui_session', 'host', 'host_private', ['node_view_state'], true, 'diagnostic'),
  policy('node_view_state', null, 'ui_session', 'host', 'host_private', ['node_view_state'], true, 'diagnostic'),
...
export const SYNC_POLICY_DEVICE_PRIVATE_OBJECT_TYPES = uniqueObjectTypes(
  SYNC_OBJECT_POLICIES.filter((item) => item.scope === 'device')
);

export const SYNC_POLICY_HOST_PRIVATE_OBJECT_TYPES = uniqueObjectTypes(
  SYNC_OBJECT_POLICIES.filter((item) => item.scope === 'host')
);
